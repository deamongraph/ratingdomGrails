...
public class YoutubeHistoryV3 {
   ...
   /* GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(
      ...
      try {
          clientSecrets = GoogleClientSecrets.load(JSON_FACTORY,
                  new InputStreamReader(YoutubeHistoryV3.class.getResourceAsStream("client_secrets.json")));
  ...
  static def getCategoryTitle(categoryID){
     ...
     //modificado c.id por c.title
  static def updateUploaders(){
	  db.eachRow("""SELECT h.uploader as cid
	  FROM historyv3 h
	  LEFT JOIN uploaders c ON h.uploader = c.title
	  WHERE c.id IS NULL
	  GROUP BY h.uploader
	  ORDER BY h.uploader"""){
  ...
  public static void main(String[] args) {
    ...
    try {
	  ...
	  //db
	    ...
	    db = Sql.newInstance('jdbc:postgresql://localhost:5432/youtube', props , 'org.postgresql.Driver')
